...
**`surplus_vs_merchant_avg` removed:** This interaction feature computed `surplus_quantity - merchant_avg_surplus` using the current row's own target, which is **current-row target leakage**. It has been removed from the feature set entirely. The model uses **31 raw feature fields, expanded into 47 model input columns after one-hot encoding** for categorical variables (product_category, merchant_type, storage_type).
...
| **Deployment readiness**    | Jupyter notebook                        | Interactive Streamlit dashboard, documented pipeline, 75 passing tests           |
...
| Claim                               | Evidence                                                       |
| ----------------------------------- | -------------------------------------------------------------- |
| XGBoost MAE 0.6355 (temporal holdout) | `outputs/model_comparison.csv`                           |
| 57% improvement over baseline       | Computed: (1.49 − 0.6355) / 1.49 on temporal holdout            |
...
| 75 passing unit tests               | `pytest tests/unit/ -v` output                                 |
| RANDOM_SEED=42                      | `src/train_model.py`, `data/generate_synthetic_data.py`        |
| 31 raw features → 47 model columns | `src/feature_engineering.py` `get_feature_columns()`                  |
